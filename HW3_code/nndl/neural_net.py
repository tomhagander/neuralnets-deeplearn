...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    h1 = W1.dot(X.T) + b1.reshape(-1, 1)
    relued = np.maximum(h1, np.zeros_like(h1))
    scores = (W2.dot(relued) + b2.reshape(-1, 1)).T
    pass
    ...
    loss1 = (1/N)*np.sum(np.log(np.sum(np.exp(scores),axis=1)),axis=0) 
    loss2 = - (1/N)*np.sum(scores[np.arange(len(y)), y])
    regloss = 0.5*reg*(np.linalg.norm(W1)**2 + np.linalg.norm(W2)**2)

    loss = loss1 + loss2 + regloss
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
