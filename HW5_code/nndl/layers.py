...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        mu = x.mean(axis=0)
        sig_squared = x.var(axis=0)
        x_hat = (x - mu)/(np.sqrt(sig_squared + eps))
        out = np.multiply(x_hat, gamma) + beta

        running_mean = momentum * running_mean + (1-momentum) * mu
        running_var = momentum * running_var + (1-momentum) * sig_squared
        cache = (x_hat, gamma, sig_squared, eps, x, mu)
...
def batchnorm_backward(dout, cache):
    ...
    #cache unpack
    x_hat, gamma, sig_squared, eps, x, mu = cache
    ...
    t1 = dx_hat/np.sqrt(sig_squared + eps)
    dsig_squared = np.sum(-0.5*np.multiply(x - mu, dx_hat)/(sig_squared + eps)**(3/2), axis=0)
    t2 = 2*np.multiply(x - mu, dsig_squared)/N
    dmu = -np.sum(dx_hat, axis=0)/np.sqrt(sig_squared + eps)
    t3 = dmu/N
